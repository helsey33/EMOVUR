{
  ...
  class Slide {
    constructor(el) {
      ...
      this.DOM.preview = {
        ...
        revealer: this.DOM.el.querySelector(
          ".preview__img-wrap > .preview__img-reveal"
        ),
    ...
    toggle(action, direction) {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (direction === "left" || direction === "right") {
          ...
          numberOpts.startAt =
            action === "show"
              ...
              : {};
          numberOpts.x =
            action === "hide" ? (direction === "left" ? "50%" : "-50%") : "0%";
        } else {
          revealerOpts.startAt =
            action === "hide"
              ? { x: "0%", y: direction === "down" ? "-100%" : "100%" }
              : { x: "0%", y: "0%" };
          revealerOpts.y =
        ...
        TweenMax.to(
          this.DOM.title,
          this.config.animation.duration * 1.5,
          titleOpts
        );
        TweenMax.to(
          this.DOM.number,
    ...
    togglePreview(action) {
      return new Promise((resolve, reject) => {
        ...
        TweenMax.to(
          [this.DOM.preview.title, this.DOM.preview.content],
          this.config.animation.duration,
          {
            delay: action === "hide" ? 0 : this.config.animation.duration / 2,
            ease: this.config.animation.ease,
  ...
  class Slideshow {
    constructor(el) {
      ...
      this.DOM.prevCtrl = this.DOM.el.querySelector(".slidenav__itemM--prev");
      this.DOM.nextCtrl = this.DOM.el.querySelector(".slidenav__itemM--next");
    ...
    togglePreview(action) {
      ...
      const processing =
        action === "enter"
          ? [
              this.slides[this.current].hide("up"),
              this.slides[this.current].showPreview()
            ]
          : [
              this.slides[this.current].show("down"),
              this.slides[this.current].hidePreview()
            ];
    ...
    toggleNavCtrls(action) {
      TweenMax.to([this.DOM.prevCtrl, this.DOM.nextCtrl], 0.5, {
        ...
        onStart: () =>
          (this.DOM.prevCtrl.style.pointerEvents = this.DOM.nextCtrl.style.pointerEvents =
            action === "enter" ? "none" : "auto")
      });
    }
  ...
  imagesLoaded(
    document.querySelectorAll([".slide__img", ".slide__imgS"]),
    { background: true },
    () => {
      console.log("removed");
